...
app.post("/student/login", (req, res) => {
    ...
    db.query("SELECT * FROM student WHERE email=?", [email], async (err, result) => {
        if (err || result.length === 0) return res.json({ success: false, message: "Student not found" });
        ...
        res.json({
            success: true,
            student: { name: result[0].name, email: result[0].email }
        });
    });
});
...
app.post("/teacher/login", (req, res) => {
    ...
    db.query("SELECT * FROM teacher WHERE email=?", [email], async (err, result) => {
        if (err || result.length === 0) return res.json({ success: false, message: "Teacher not found" });
        ...
        res.json({
            success: true,
            teacher: { name: result[0].name, email: result[0].email }
        });
    });
});
...
app.post("/course/add", (req, res) => {
    ...
    db.query("INSERT INTO courses (course_name) VALUES (?)", [course_name], err => {
...
app.get("/assignments/:teacherName", (req, res) => {
    db.query("SELECT * FROM assignments WHERE teacher_name=?", [req.params.teacherName],
        (err, result) => {
            if (err) return res.json({ success: false });
...
app.post("/notes", (req, res) => {
    ...
    db.query("INSERT INTO notes (teacher_name, content) VALUES (?,?)", [teacherName, content],
        err => {
            if (err) return res.json({ success: false });
            res.json({ success: true });
        });
